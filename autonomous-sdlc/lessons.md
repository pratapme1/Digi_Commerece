...
- 2026-04-05 `015-m7-canonical-attendee-content`: when browser E2E depends on an exported web bundle, rebuild that bundle before treating Playwright failures as application regressions, and keep pilot/readiness checks pointed at the real production surface after a prototype is replaced.
- 2026-04-05 `016-flutter-host-replatform-and-scope-correction`: Flutter web browser coverage is reliable only when tests bootstrap semantics explicitly, target `ChoiceChip` controls by their checkbox role, and replace controller-backed field values with a select-all-and-insert flow instead of assuming DOM-style `fill()` semantics.
